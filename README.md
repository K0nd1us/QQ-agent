...
- 思考开关：混合推理模型（Qwen3 / GLM / DeepSeek 等）可关掉思维链，请求带 `enable_thinking=false`，更省 token
- 预算保险丝：设今日成本上限（元），达到后自动暂停，调高后一键恢复
...
- 禁言/风控熔断：QQ 返回限制发言类错误时，该会话按配置时长暂停发送，避免反复撞墙；可手动清除
